...
float correlation_coefficient(float *image, int num_cols,int patch_x, int patch_y,int i,int j,int k,int l, int removing_mean)
{
   ...
   {
      // offset into image for current line of patch A / patch B
      float *patchA = image+(i  +m)*num_cols+j;
      float *patchB = image+(i+k+m)*num_cols+j+l;
      for (n=-patch_y;n<=patch_y;n++)
      {
          sum_ab += patchA[n] * patchB[n];
	  sum_aa += patchA[n] * patchA[n];
	  sum_bb += patchB[n] * patchB[n];
	  sum_a  += patchA[n];
	  sum_b  += patchB[n];
      }
   }
   ...
   b_mean = sum_b / length;
   if (removing_mean == 1)
      corr_ef = (sum_ab - length*a_mean*b_mean)
              / (sqrtf(sum_aa-length*a_mean*a_mean)*sqrtf(sum_bb-length*b_mean*b_mean));
   else
...
void calc_descriptor(float *image, float*descriptor, int num_rows, int num_cols,
                     int patch_x, int patch_y, int region_x, int region_y,
                     int region_grid_x, int region_grid_y,
                     int n_descriptors, int step, int num_rows_desp, int num_cols_desp, int removing_mean)
{
#pragma omp parallel
   {
      int i,j;
